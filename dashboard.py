...
if "grade_num" not in df_raw.columns:
    df_raw["grade_num"] = df_raw["grade"].map(
        {"A": 1, "B": 2, "C": 3, "D": 4, "E": 5, "F": 6, "G": 7}
    )
if "term_months" not in df_raw.columns:
    df_raw["term_months"] = (
        df_raw["term"].str.replace(" months", "").str.strip().astype(float)
    )
...
df_raw["installment_to_income"] = df_raw["installment"] / (
    df_raw["annual_inc"].clip(lower=1) / 12
)
if "earliest_cr_line" in df_raw.columns:
    df_raw["earliest_cr_line"] = pd.to_datetime(
        df_raw["earliest_cr_line"], format="%b-%Y", errors="coerce"
    )
    df_raw["credit_history_years"] = (
        pd.Timestamp("2018-12-31") - df_raw["earliest_cr_line"]
    ).dt.days / 365.25
else:
...
if os.path.exists(os.path.join(MODELS_DIR, "xgb.pkl")):
    ...
    lgbm = (
        joblib.load(f"{MODELS_DIR}/lgbm.pkl")
        if os.path.exists(f"{MODELS_DIR}/lgbm.pkl")
        else None
    )
    ...
    f1_scores = [
        f1_score(y_test, (proba_xgb >= t).astype(int), pos_label=1) for t in thresholds
    ]
...
if all(os.path.exists(p) for p in SHAP_PNG_FILES.values()):
    ...
    shap.summary_plot(
        shap_values.values, X_shap, plot_type="bar", max_display=15, show=False
    )
    ...
    plt.title(
        f"High-Risk Borrower  (p = {proba_shap[high_risk_idx]:.1%})", fontweight="bold"
    )
    plt.tight_layout()
    ...
    plt.title(
        f"Low-Risk Borrower  (p = {proba_shap[low_risk_idx]:.1%})", fontweight="bold"
    )
    plt.tight_layout()
...
fig_pie = px.pie(
    ...
    title="Binary Target: Default vs No Default",
    hole=0.4,
)
...
home_dr = (
    df[df["home_ownership"].isin(["OWN", "MORTGAGE", "RENT"])]
...
fig_corr_bar = go.Figure(
    go.Bar(
        ...
        text=[f"{v:.2f}" for v in corr_target.values],
...
fig_pr.update_layout(
    ...
    yaxis_title="Precision (flagged loans that truly default)",
    plot_bgcolor=C["bg"],
)
...
decile_sum = (
    decile_df2.groupby("decile")
    .agg(count=("actual", "count"), actual_default_rate=("actual", "mean"))
    .reset_index()
)
fig_decile = px.bar(
    decile_sum,
...
_sorted_results = results_df.sort_values("AUC-ROC", ascending=False).reset_index(
    drop=True
)
...
def _metric_cell(val, best_val):
    ...
    cell_style = (
        {"fontWeight": "700", "color": "#1a9e6b"} if is_best else {"color": "#2d3436"}
    )
...
fig_summary.add_vline(
    x=0.5, line_dash="dash", line_color="red", annotation_text="Random = 0.5"
)
...
def section_header(title, subtitle=None):
    return html.Div(
        [
            ...
            html.P(subtitle, className="text-muted mb-2", style={"fontSize": "0.88rem"})
            if subtitle
            else None,
        ]
    )
...
def model_info_badge(model_key):
    ...
    tip_content = html.Div(
        [
            ...
            html.P(
                [html.Strong("How it works: "), m["how"]],
                style={"fontSize": "0.82rem", "marginBottom": "6px"},
            ),
            html.P(
                [html.Strong("How to read results: "), m["read"]],
                style={"fontSize": "0.82rem", "marginBottom": "6px"},
            ),
            html.P(
                [
    ...
    [
        dbc.Col(
            kpi_card(
                ...
                f"First {len(df):,} loans with known outcome",
                "#3498db",
            ),
            md=3,
        ),
        dbc.Col(
            kpi_card(
                "Features used",
                "28 variables",
                "31 selected · 3 derived · origination-only",
                "#27ae60",
            ),
            md=3,
        ),
        ...
        dbc.Col(
            kpi_card(
                "Key challenge",
                "Class imbalance",
                f"~{1 - AVG_DEFAULT:.0%} no default vs ~{AVG_DEFAULT:.0%} default",
                "#e67e22",
            ),
            md=3,
        ),
...
tab_eda = dbc.Container(
    [
        ...
        insight_box(
            f"The dataset has a {AVG_DEFAULT:.1%} default rate — 1 in every 5 loans was not fully repaid. "
            ...
            [
                dbc.Col(
                    [
                        ...
                        insight_box(
                            "Grade is the strongest single predictor. "
                            "Grade G has ~6× more defaults than Grade A.",
                            C["red"],
                        ),
                    ],
                    md=6,
                ...
                dbc.Col(
                    [
                        ...
                        insight_box(
                            "small_business is the riskiest purpose. "
                            "wedding and major_purchase have the lowest default rates.",
                            C["blue"],
                        ),
                    ],
                    md=6,
        ...
        dbc.Row(
            [
                dbc.Col(
                    [
                        dcc.Graph(figure=fig_home),
                        insight_box(
                            "Hypothesis confirmed: OWN < MORTGAGE < RENT. "
                            "Owning a home signals financial stability and accumulated assets."
                        ),
                    ],
                    md=6,
                ),
                dbc.Col(
                    [
                        dcc.Graph(figure=fig_verif),
                        insight_box(
                            "Counterintuitive: 'Verified' borrowers default MORE. "
        ...
        concept_box(
            "How to read KDE / histogram plots",
        ...
        insight_box(
            "FICO score, interest rate, and DTI show the clearest separation. "
        ...
        insight_box(
            "Top predictors: int_rate (+), grade_num (+), fico_avg (−), revol_util (+), dti (+). "
            "int_rate and grade_num are ~0.9 correlated — multicollinearity to watch in Logistic Regression."
        ),
        html.Hr(),
        # ── NEW: Time series ──────────────────────────────────────────
        section_header(
            "Default Rate & Loan Volume over Time",
        ...
        insight_box(
            "Loans originated in 2007-2009 show dramatically higher default rates — the fingerprint of the financial crisis. "
        ...
        section_header(
            "Geographic Distribution — Default Rate by State",
        ...
        insight_box(
            "States in the South and Midwest tend to show above-average default rates. "
        ...
        section_header(
            "Sub-Grade Risk Heatmap (A1 to G5)",
        ...
        concept_box(
            "Why sub-grade matters for modeling",
        ...
        insight_box(
            "The heatmap should show a clean gradient from top-left (A1, lowest risk) to bottom-right (G5, highest risk). "
        ...
        insight_box(
            "small_business loans are both larger than average and riskier. "
...
tab_model = dbc.Container(
    [
        ...
        html.Div(
            [
                html.P(
                    "Metrics on a stratified hold-out test set (20% of ~102k loans). "
        ...
        concept_box(
            "AUC-ROC and Gini — why these metrics?",
                    ...
                    [
                        html.Div(
                            [
                                ...
                                model_info_badge("dt"),
                                html.Span(
                                    " Decision Tree  ", style={"fontSize": "0.9rem"}
                                ),
                                model_info_badge("rf"),
                                html.Span(
                                    " Random Forest  ", style={"fontSize": "0.9rem"}
                                ),
                                model_info_badge("xgb"),
                                html.Span(" XGBoost  ", style={"fontSize": "0.9rem"}),
                                model_info_badge("lgbm"),
                                html.Span(" LightGBM", style={"fontSize": "0.9rem"}),
                            ],
                            className="mb-2",
                        ),
                    ]
        ...
        insight_box(
            "Ranking: Decision Tree < Logistic Regression < Random Forest < XGBoost. "
        ...
        insight_box(
            f"The optimal threshold for XGBoost is {OPTIMAL_THRESHOLD:.2f} — not the intuitive 0.5. "
            "With 80/20 imbalanced data, most defaults score well below 0.5, "
            "so using 0.5 as the cutoff misses a large share of them."
        ),
        html.Hr(),
        section_header(
            "Confusion Matrix — Threshold Sensitivity (XGBoost)",
        ...
        insight_box(
            "False Negatives (missed defaults) = financial loss for the bank. "
            "False Positives (good loans rejected) = lost revenue. "
            "Lowering the threshold reduces FN but increases FP. Neither is free."
        ),
        html.Hr(),
        section_header(
            "Feature Importance", "Which variables does the model rely on most?"
        ),
        dbc.Row(
            [
                dbc.Col(
                            ...
                            [
                                dbc.RadioItems(
                                    ...
                                    options=[
                                        ...
                                        {
                                            "label": html.Span(
                                                ["XGBoost ", model_info_badge("xgb")]
                                            ),
                                            "value": "xgb",
                                        },
                                        {
                                            "label": html.Span(
                                                ["LightGBM ", model_info_badge("lgbm")]
                                            ),
                                            "value": "lgbm",
                                        },
                                    ],
        ...
        insight_box(
            "Both models agree: int_rate, grade_num, fico_avg, dti are the top features. "
            ...
            [
                dbc.Col(
                    [
                        ...
                        html.Small(
                            " (click to generate — takes ~10 seconds)",
        ...
        insight_box(
            "High int_rate (red, right) increases default risk. High fico_avg (red, left) decreases it. "
            "All directions match economic intuition — a model with contradictory SHAP directions "
            "would be a red flag for data leakage.",
            C["blue"],
        ),
        dbc.Row(
            [
        ...
        insight_box(
            "Case A: no single factor disqualifies — it is the combination of high int_rate + "
        ...
        html.Div(
            id="cv-results-section",
            children=[
                html.P(
                    "CV results not available — run save_models.ipynb with cv_results included in eval_data.pkl.",
                    className="text-muted",
                ...
                if CV_RESULTS is None
                    ...
                    [
                            ...
                            [
                                ...
                                html.Tbody(
                                    [
                                        ...
                                        for row in sorted(
                                            CV_RESULTS, key=lambda r: -r["mean_auc"]
                                        )
                                    ]
                                ),
                            ],
                        ...
                        html.Small(
                            "CV scores computed on training data only. Test set was not used during cross-validation.",
                            className="text-muted",
        ...
        insight_box(
            "Cross-validation confirms the ranking from the test set — XGBoost and LightGBM outperform "
        ...
        insight_box(
            "Default rate increases consistently from Decile 1 to Decile 10. "
    ...
    [
                    ...
                    [
                        ...
                        inp_row(
                            [
                                ...
                                dbc.Col(
                                    [
                                        tooltip_label("Term", TIPS["term"], "term"),
                                        dcc.Dropdown(
                                            id="inp-term",
                                            options=[
                        ...
                        inp_row(
                            [
                                dbc.Col(
                                    [
                                        tooltip_label("Grade", TIPS["grade"], "grade"),
                                        dcc.Dropdown(
                                            id="inp-grade",
                                            options=[
                                                {"label": g, "value": g}
                        ...
                        inp_row(
                            [
                                ...
                                dbc.Col(
                                    [
                                        tooltip_label(
                                            "Public Records", TIPS["pub_rec"], "pub-rec"
                                        ),
                                        dbc.Input(
                                            id="inp-pub-rec",
                                            type="number",
                        ...
                        inp_row(
                            [
                                dbc.Col(
                                    [
                                        tooltip_label(
                                            f"Decision Threshold (optimal = {OPTIMAL_THRESHOLD:.2f})",
                                            TIPS["threshold"],
                                            "threshold",
                                        ),
                                        dcc.Slider(
                        ...
                        dbc.Spinner(
                            [
                                html.Div(
                                    id="calc-output",
                                    children=dbc.Alert(
                                        "Fill in the form and click 'Calculate Risk' to get the prediction.",
                                        color="secondary",
                                    ),
                                ),
                                html.Hr(),
                                ...
                                html.Hr(),
                                html.H6(
                                    "What drove this prediction? (SHAP waterfall)",
                                    className="mt-2",
                                ),
                                html.Small(
                                    "Red bars pushed toward default. Blue bars pushed away from default.",
                                    className="text-muted",
                                ),
                                html.Br(),
        ...
        html.Div(
            [
                ...
                html.P(
                    "How does the model behave across all borrowers? "
                ...
                dbc.Row(
                    [
                        dbc.Col(
                            [
                                html.H6(
                                    "Feature Impact & Direction (Beeswarm)",
                                    className="text-center mb-2",
                                    style={"fontSize": "0.88rem", "color": "#636e72"},
                                ),
                                html.Img(
                                    id="calc-shap-beeswarm",
                                    src=SHAP_BEESWARM,
                                    style={
                                        "width": "90%",
                                        "display": "block",
                ...
                insight_box(
                    "High int_rate and high grade_num push predictions toward default. "
    ...
    [
        ...
        html.Div(
            [
                ...
                html.Span(
                    "  ·  Loans excluded: 'Current' status (outcome unknown)  ·  "
...
def update_timeseries(year_range):
    ...
    agg = (
        ts.groupby("period")
        .agg(count=("default", "count"), default_rate=("default", "mean"))
        .reset_index()
    )
...
def update_purpose_amount(amount_range):
    filtered = df[
        (df["loan_amnt"] >= amount_range[0]) & (df["loan_amnt"] <= amount_range[1])
    ]
...
def update_dist(col):
    try:
        ...
        fig_h = px.histogram(
            ...
            opacity=0.6,
            nbins=60,
            labels={col: label, "default_label": ""},
...
def update_cm(threshold):
    ...
    fig = px.imshow(
        ...
        title=f"Confusion Matrix — XGBoost (threshold = {threshold})",
        aspect="auto",
    )
        ...
        [
            html.Tbody(
                [
                    ...
                    html.Tr(
                        [html.Td("Precision"), html.Td(f"{rep['1']['precision']:.3f}")]
                    ),
                    html.Tr([html.Td("Recall"), html.Td(f"{rep['1']['recall']:.3f}")]),
                    html.Tr(
                        [html.Td("F1-Score"), html.Td(f"{rep['1']['f1-score']:.3f}")]
                    ),
...
def update_calculator(
    ...
    if decision == "DEFAULT":
        ...
        dec_emoji = "✅"
        dec_note = (
            "Default probability is well below the threshold. Loan looks low risk."
        )
                ...
                [
                        ...
                        [
                            ...
                            dbc.Col(
                                [
                                    ...
                                    html.Br(),
                                    html.Small(
                                        f"Decision threshold: {round(threshold, 2)}",
                                        className="text-muted",
                                    ),
                                ],
                                md=7,
                    ...
                    dbc.Row(
                        [
                            ...
                            dbc.Col(
                                [
                                    html.Small("Int Rate", className="text-muted"),
                                    html.H5(f"{int_rate}%"),
                                ],
                                md=3,
                            ),
                            dbc.Col(
                                [
                                    html.Small("FICO", className="text-muted"),
                                    html.H5(fico),
                                ],
                                md=3,
                            ),
                            dbc.Col(
                                [
                                    html.Small("DTI", className="text-muted"),
                                    html.H5(dti),
                                ],
                                md=3,
                            ),
    ...
    for col_idx, (col_name, user_val, col_label) in enumerate(ctx_pairs, start=1):
        ...
        fig_ctx.update_xaxes(
            title_text=col_label, row=1, col=col_idx, title_font=dict(size=11)
        )
    ...
    contrib_pct = (
        (contrib_raw / total_contrib * 100) if total_contrib > 0 else contrib_raw
    )
        ...
        rows.append(
            html.Tr(
                [
                    ...
                    html.Td(
                        _fmt_val(feat, user_val),
                        style={"fontSize": "0.8rem", "textAlign": "right"},
                    ),
                    html.Td(
                        _fmt_val(feat, med_val),
                        style={
                            "fontSize": "0.8rem",
                            "textAlign": "right",
                            "color": C["grey"],
                    ...
                    html.Td(direction, style={"fontSize": "0.78rem", "color": color}),
                ]
            )
        )
